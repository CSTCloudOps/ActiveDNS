...
def main(days=1):
    ...
    zone_index = f"CREATE INDEX zone_index ON {db_table_name} (zone);"
    ip_index = f"CREATE INDEX ip_index ON {db_table_name} (data);"
    db.execu(zone_index)
    db.execu(ip_index)
    domain_list = []
    for ip in new_unuseful_ip:
        query = f"select zone from {db_table_name} where data={ip};"
        query_result = db.query(query)
        print(query_result)
        delete = f"DELETE FROM {db_table_name} WHERE data={ip};"
        # db.execu(delete)

        for item in query_result:
            domain = item[0]
            domain_list.append(domain)
            query_cnt_zone = f"SELECT COUNT(*) FROM {db_table_name} WHERE zone = {domain};"
            ...
            if cnt_zone[0][0] == 0:
                ...
                for ip in ips_rtt:
                    if ips_rtt[ip]:
                        insert_table_domain_ip_word = "INSERT INTO \'{dns_records}\' (zone,host,type,data,ttl,retry,rtt,score1,score2,domain_query_time,ip_update_time) VALUES (\'{zone}\',\'{host}\',\'{type}\',\'{data}\',{ttl},{retry},{rtt},{score1},{score2},{time},{time});".format(dns_records=db_table_name,zone=domain, host='@', type='A', data=ip, ttl=80, retry=10, rtt=ips_rtt[ip], score1=ips_rtt[ip], score2=ips_rtt[ip], time=int(time.time()))
